...
export default class JsonSchemaForm extends React.PureComponent {
  ...
  render() {
    const { className = '', schema, dataPath, enableSumbit, disabled } = this.props;
    const { store } = this.state;
    let C;
    ...
    return (
      <GlobalStoreContext.Provider value={this.state}>
        <div className='custom-schema-form'>
          <div className={`json-schema-form ${className} ${disabled ? 'disabled' : ''}`}>
            {
              <C
                dataPath={dataPath}
                schema={schema}
            ...
            {enableSumbit && (
              <div className='item-sumbit'>
                <Button onClick={this.handleSumbit} size='large' type='primary'>
                  {getName('submit')}
                </Button>
              </div>
            )}
