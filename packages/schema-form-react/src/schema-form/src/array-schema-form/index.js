...
export default class ArrayFieldForm extends React.PureComponent {
  ...
  getColumns(config) {
    ...
    if (enableDrag && this.props.value.length > 10) {
      settingIcons.push({
        render: (text, record) => {
          ...
          return (
            <Tooltip key='arrow-right' title={getName('jump_to_a_line')}>
              <ArrowRightOutlined onClick={this.openGoto(index)} />
            </Tooltip>
          );
        }
      });
    ...
    if (Array.isArray(advFields) && advFields.length > 0) {
      settingIcons.unshift({
        render: (text, record) => {
          const index = record.key;
          return (
            <Tooltip key='setting' title={getName('advance-setting')}>
              <SettingOutlined onClick={this.openModal(index)} />
            </Tooltip>
          );
        }
      });
    ...
    const settingColumns = [
      {
        ...
        render: (text, record) => {
          return (
            <div className='setting-column-item'>
    ...
    if (enableDrag) {
      initColumns.push({
        ...
        render: () => {
          return (
            <div key={'drag'} is-drag='true' className='setting-column-item-drag'>
              <DragOutlined />
            </div>
          );
        }
  ...
  objectKeyRender = ({ el, errorMessage }) => {
    return (
      <div>
        {el}
  ...
  render() {
    const { uniqueField, schema, value = [], disabled } = this.props;
        ...
        <div className='header'>
          ...
          {uniqueField && (
            <div className='fast-edit'>
              <Button onClick={this.openFastEdit} disabled={disabled}>
                {getName('quick-edit')}
              </Button>
            </div>
          )}
          <div className='add-button'>
            <Button onClick={this.handleAdd()} disabled={disabled} type='primary'>
              {getName('add')}
            </Button>
          </div>
        </div>
